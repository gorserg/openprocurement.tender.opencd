...
from openprocurement.tender.opencd.tests.base import BaseTenderContentWebTest, test_bids, test_lots
...
class TenderQualificationComplaintDocumentResourceTest(BaseTenderContentWebTest):
    ...
    def test_patch_tender_qualification_complaint_document(self):
        ...
        response = self.app.put('/tenders/{}/qualifications/{}/complaints/{}/documents/{}?acc_token={}'.format(self.tender_id, self.qualification_id, self.complaint_id, doc_id, self.complaint_owner_token), 'content', content_type='application/msword', status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
